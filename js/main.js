...
async function parseMusicLink(link) {
  try {
    ...
    // 自动选择代理：部署到 Vercel 用同源接口，本地开发回退到第三方代理
    const isVercel = location.hostname.endsWith('.vercel.app') || location.hostname.endsWith('.vercel.sh');
    const corsProxy = isVercel
      ? '/api/cors-proxy?url='
      : 'http://qiuyu520.fun/cors/?url=';
    const proxyUrl = corsProxy + encodeURIComponent(url);
    console.log('[汽水音乐解析] 使用代理:', proxyUrl);
        ...
        try {
          ...
          if (match && match[1]) {
            ...
            const musicInfo =
              ...
              jsonData?.loaderData?.album_page ||
              jsonData?.loaderData?.playlist_page ||
              {};

            // 尝试多个可能的音频字段路径
            const audioSources = [
              musicInfo?.audioWithLyricsOption,
              musicInfo?.trackInfo,
            ...
            for (const source of audioSources) {
              ...
              if (urlFields.length > 0) {
                const firstUrl = urlFields[0];
                musicData = {
                  title: source?.trackName || source?.name || musicInfo?.trackName || "未知歌曲",
                  artist: source?.artistName || source?.artist?.name || "未知歌手",
                  album: source?.trackInfo?.album?.name || source?.album?.name || "未知专辑",
                  cover: source?.coverURL || source?.cover || "",
                  url: encodeURI(decodeURI(firstUrl)),
                };

                console.log("[汽水音乐解析] 找到音频URL字段，使用:", firstUrl);
                console.log("[汽水音乐解析] musicData:", musicData);
                ...
                fetch(musicData.url, {
                  ...
                  .then(blob => {
                    ...
                    console.log("[汽水音乐解析] 音频加载成功，大小:", (blob.size / 1024 / 1024).toFixed(2) + "MB");
                  })
                  .catch(err => {
                    console.error("[汽水音乐解析] 音频加载失败:", err);
                  });
